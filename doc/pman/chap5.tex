...
\verb+SegmentsIntersect+&\verb+<ax,ay,bx,by,cx,cy,dx,dy>+&Determines whether line segment ax intersects cd.\\
...
\verb+MakeAttneave+&\verb+<radius>+,\verb+numpoints+,\verb+minangle+,\verb+maxangle+&Makes
a complex ``Attneave'' polygon\\
...
\verb+MakeGabor+&\verb+[size,freq,sd, angle,phase,bglev]+&Creates a 'gabor patch' with specified parameters\\
\addlinespace[0.2cm] 
\cmidrule(l{.5cm}r{.5cm}){1-3} 
...
\verb+WaitForMouseClickWithTimeout+ &\verb+<timeout>+ &Waits until any of the mouse buttons is pressed, or a prespecified timout has elapsed. \\ 
...
\verb+Sample+ & \verb+<list>+ & Picks a single item randomly from <list>.\\
...
\verb+RemoveSubset+ &\verb+<list>+ \verb+<list-of-pos>+ & Removes items at positions <list-of-pos> from a list.\\ 
\verb+ListToString+ &\verb+<list>+ & Concatenates all elements of a list into a single string\\
\verb+Insert+ & \verb+<list>,<item>,<pos>+ & Inserts <item> into <list> at <pos>\\
